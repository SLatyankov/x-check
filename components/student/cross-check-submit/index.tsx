...
const CrossCheckSubmitPage: React.FC<PropsCrossCheckPage> = ({
  ...
  const bufID = 'NlAJHlGjoXa4VrIKP1MNQeiAfW62';
  ...
  const selectTask = (selectTaskID: string) => {
    ...
    if (selectCourseData.length !== 0) {
      ...
      if (
        ...
        setCheckTask(
          createWorkDone(newCrossCheckSubmit.activeCourseData.task, activeUser, usersData).selfTest
        );
        setReviewer(
  ...
  const selectReviewer = (selectReviewer: IStudent | IMentor) => {
    ...
    if (findCheckTask.length !== 0) {
      ...
      if (crossCheckSubmit.activeCourseData.workDone.mentorCheck.checkerID !== selectReviewer.id) {
